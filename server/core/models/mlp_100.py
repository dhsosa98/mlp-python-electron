...
def trainModelA(lr=0.5, momentum=0.5, epoch=20, hl_topology = [5], val_percentage=0.1, save=False):
  ...
  model_name = 'model100,'+str(lr)+','+str(momentum)+','+str(epoch)+','+'; '.join(list(map(str, topology)))+','+str(val_percentage)

  path = os.path.dirname(__file__)+'/../datasets/letras_distorsionadas100.csv'
